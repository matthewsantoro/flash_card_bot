...
async def learnig_collections(callback: CallbackQuery, state: FSMContext):
    ...
    if study_cards:
        ...
        await callback.message.edit_text(f'У вас нет карточек для узучения', reply_markup=keyboard)
...
async def startig_learn(callback: CallbackQuery, state: FSMContext):
    ...
    await show_study_card(callback.message, cards=study_cards[0], state=state)
...
async def checking_result(callback: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    study_cards= data['study_card']
    study_card: Card = study_cards.popleft()
    if study_card.level == 1 and not is_sucseed:
    ...
    await state.update_data(study_cards=study_cards)
    if study_cards:        
        await show_study_card(callback.message, cards=study_cards, state=state)
    else: 
        cards = data['cards']    
        card = data['card']
        index = cards.index(card)
        await state.set_data(CardState.view_card)
        await show_card(callback.message, index=index, cards=cards, state=state)
